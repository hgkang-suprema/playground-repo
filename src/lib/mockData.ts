/**
 * 영업 KPI 대시보드를 위한 메모리 기반(mock) 데이터베이스입니다.
 * - 외부 DB 사용 금지: 런타임 메모리 전용(데모/개발 목적)
 * - 날짜는 ISO 문자열(YYYY-MM-DD) 형식으로 표기합니다.
 * - 금액은 숫자 타입(원 단위)으로 저장합니다.
 * - 프론트엔드 스타일 가이드: shadcn/ui의 시맨틱 토큰을 사용하세요
 *   (예: bg-background, text-foreground 등). 색상 하드코딩 금지.
 *
 * 사용 안내:
 * - 이 파일은 데모 및 로컬 개발용으로만 사용됩니다. 서버 재시작 시 데이터는 초기화됩니다.
 * - API 라우트에서는 이 모듈의 쿼리/업서트 유틸을 호출하여 테스트 데이터를 제공합니다.
 */
